...
% \setmonofont[%
% AutoFakeSlant,                  %内部で擬似的な斜体を作る
% BoldItalicFeatures={FakeSlant}, %内部で擬似的なイタリック体を作る
% Scale=MatchLowercase]
% {Inconsolatazi4} % 等幅フォントにInconsolataを指定
...
\subsection{ルビ}
\index{ルビ}
\ltjruby{黄}{き}と\ltjruby{赤}{あか}と
\ltjruby{緑}{みどり}の
\ltjruby{高|価}{こう|か}で
\ltjruby{貴|重}{き|ちょう}なものを
\ltjruby{購|入}{こう|にゅう}しました。
\ltjruby{閑話休題}{それはさておき}、
\ltjruby{昨日}{きのう}は
\ltjruby{心太}{ところてん}を
ありがとう。
